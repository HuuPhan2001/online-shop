...
public interface ICategoryRepository extends IRepository<Category> {
    @Query("select c from Category c where ((:type is null) or c.categoryTypeId = (:type)) AND UPPER(c.name) LIKE %:name%")
    ...
    @Query("SELECT c FROM Category c WHERE c.status <> 'DELETED' " +
            ...
            " AND (:text IS NULL OR ((LOWER(c.code) LIKE %:text%) OR (LOWER(c.name) LIKE %:text%))) " +
            " AND (:status IS NULL OR c.status = :status)")
    Page<Category> pageCategory(Long typeId, String text, StatusEnum status, Pageable pageable);
}
